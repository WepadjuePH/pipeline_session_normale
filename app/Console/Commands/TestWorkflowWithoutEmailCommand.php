...
class TestWorkflowWithoutEmailCommand extends Command
{
    protected $signature = 'test:workflow-no-email';
    protected $description = 'Test le workflow complet sans envoyer les emails (mode log)';
    ...
    {
        $this->info('🚀 Démarrage du test workflow (sans emails)...');
        $this->newLine();
        ...
        try {
            ...
            if (!$agent) {
                $agent = User::create([
                    'nom' => 'Agent',
                    'prenom' => 'Dépôt',
                    'email' => 'agent.depot.' . uniqid() . '@example.com',
                    'password' => Hash::make('password'),
                    'role' => 'agent_depot',
                    'email_verified_at' => now(),
                ]);
            ...
            $this->info('ÉTAPE 7: Vérification des emails');
            $this->line("✅ Emails qui auraient été envoyés:");
            $this->line("   1. Email de soumission (CandidatureSubmittedNotification)");
            $this->line("   2. Email de validation (CandidatureValidatedNotification)");
            $this->newLine();
            ...
            $this->line("  - Statut: {$candidature->statut}");
            $this->newLine();
            $this->line('Note: Les emails n\'ont pas été envoyés (mode test)');
