...
class TestInteractiveWorkflowCommand extends Command
{
    protected $signature = 'test:interactive';
    protected $description = 'Test interactif: inscription, vérification avec code, enroulement complet';

    public function handle()
    {
        $this->info('=== TEST INTERACTIF COMPLET ===');
        $this->newLine();

        // Étape 1: Inscription
        $user_data = $this->step1_inscription();

        // Étape 2: Demander le code de vérification
        $code = $this->step2_demander_code();
        ...
        $this->info('=== TEST INTERACTIF TERMINÉ ===');
        $this->newLine();
    }
    ...
    private function step2_demander_code()
    {
        $this->info('📋 ÉTAPE 2: VÉRIFICATION EMAIL');
        $this->line('─────────────────────────────────────');

        $this->line("✓ Un email de vérification a été envoyé à: hybreltapdur7@gmail.com");
        $this->line("✓ Vérifie ton email et fournis le code de vérification");
        $this->newLine();

        $code = $this->ask('Quel est le code de vérification reçu par email?');
